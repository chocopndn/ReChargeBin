...
unsigned long extendTime(unsigned long endTime, int sec)
{
  unsigned long now = millis();
  unsigned long base = (endTime > now) ? endTime : now;
  return base + (unsigned long)sec * 1000UL;
}
...
void loop()
{
  ...
  static int last1 = HIGH, last2 = HIGH, last3 = HIGH;

  int cur1 = digitalRead(btn1);
  int cur2 = digitalRead(btn2);
  int cur3 = digitalRead(btn3);

  bool canAdd = waitingSelection && !selectionUsed;

  if (canAdd && last1 == HIGH && cur1 == LOW)
  {
    beepShort();
    if (r1run)
      r1end = extendTime(r1end, selectionSec);
    else
    ...
    Serial.println("[A1] BTN1 USED - ADD ONCE");
  }
  last1 = cur1;

  if (canAdd && last2 == HIGH && cur2 == LOW)
  {
    beepShort();
    if (r2run)
      r2end = extendTime(r2end, selectionSec);
    else
    ...
    Serial.println("[A1] BTN2 USED - ADD ONCE");
  }
  last2 = cur2;

  if (canAdd && last3 == HIGH && cur3 == LOW)
  {
    beepShort();
    if (r3run)
      r3end = extendTime(r3end, selectionSec);
    else
    ...
    Serial.println("[A1] BTN3 USED - ADD ONCE");
  }
  last3 = cur3;
  ...
  if (link.available())
  {
    ...
    if (msg == "DO_WEIGH")
    {
      ...
      if (g <= heavyLimit)
      {
        ...
        selectionUsed = false;
        Serial.println("[A1] WAITING SELECTION OPEN");
      }
      else
      {
        beepError();
